...
def index():
    ...
    print(
        f"Accès à l'index pour {user.username} avec rôle {user.groups} et permissions {user.permissions}"
    )
    if user:
    ...
    session.clear()
    flash(
        f"Connexion réussie, mais aucun rôle valide assigné pour {user.username}",
        "warning",
    )
...
def webfinger():
    ...
    issuer_href = os.getenv(
        "WEBFINGER_ISSUER", "https://authentik.sbihi.tech/application/o/tailscale/"
    )
        ...
        response = jsonify(
            {
                ...
                "error_description": "resource parameter is required and must be a valid URI",
            }
        )
    ...
    if resource not in accepted_resources:
        response = jsonify(
            {
                "error": "not_found",
                "error_description": "No information available for this resource",
            }
        )
...
def admin_dashboard():
    ...
    print(
        f"Accès au dashboard admin pour {user.username} avec rôle {user.role} et permissions {user.permissions}"
    )
    return render_template(
        "admin_dashboard.html", user=user, users=users, servers=devices
    )
...
def user_dashboard():
    ...
    now = datetime.utcnow()
    pending_request = (
        AccessRequest.query.filter_by(user_email=user.email, status="pending")
...
def api_update_user(user_id):
    ...
    return jsonify(
        {"message": "Utilisateur mis à jour avec succès", "user_id": user_id}
    )
...
def api_approve_request(request_id):
    ...
    success, msg = approve_access_request(
        access_req, "mohammed.sbihi@sbihi.tech", tailscale_tag
    )
...
def api_request_access():
    ...
    existing_request = AccessRequest.query.filter_by(
        user_email=user.email, server_id=server_id, status="pending"
    ).first()
