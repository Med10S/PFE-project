...
\begin{itemize}
	\item \textbf{Accès hors horaires normaux} : connexions à 3 h du matin ou les week-ends si habituellement ce rôle travaille 9 h-17 h.
...
\hline
\end{tabularx}
\caption{Sources de données administrateur exploitées pour l'analyse comportementale. }
\label{tab:sources_anomalies_admin}
\end{table}
Objectif: garantir une vision unifiée des événements IAM, JIT et SSH avant l'étape de détection d'anomalies.
...
\hline
\end{tabularx}
\caption{Variables explicatives retenues pour modéliser le comportement administrateur.}
\label{tab:features_anomalies_admin}
\end{table}

 Objectif: transformer des journaux bruts en indicateurs quantifiables pour Isolation Forest.
...
\begin{table}[h]
\centering
\begin{tabularx}{\textwidth}{|l|p{3cm}|l|p{5cm}|}
\hline
...
\hline
\end{tabularx}
\caption{Exemple de tableau de bord SOC pour le suivi des comptes privilégiés. }
\label{tab:dashboard_anomalies_admin}
\end{table}

Objectif: prioriser la réponse sécurité selon le score d'anomalie et le niveau de risque associé.
